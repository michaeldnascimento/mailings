...
class ListNet extends Page {
    ...
    public static function getMailingsListUser(string $list): string
    {
        ...
        $results = EntityNet::getMailingDesktop('*', null, "lista = '$list' AND id_user = $id_user AND (status_mailing IS NULL OR status_mailing = '' OR status_mailing LIKE '%OPORTUNIDADE%')", 'id DESC', '');

        //RENDERIZA O ITEM
        while($obDesktop = $results->fetchObject(EntityNet::class)){
     ...
     * @return string
     */
    public static function getListCancelado(Request $request, string $list, string $errorMessage = null): string
    {
        //STATUS > Se o errorMessage não for nulo, ele vai exibir a msg, se não ele não vai exibir nada
        $status = !is_null($errorMessage) ? Alert::getError($errorMessage) : '';
